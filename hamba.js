...
const { detectDomain, getDomainConfig, DOMAIN_CONFIGS } = require("./domain-config");
...
// ─── Domain Detection ────────────────────────────────────────
// সার্ভার চলা মাত্র domain detect করে — env var > port heuristic > localhost
const DETECTED_DOMAIN = detectDomain();
const DOMAIN_CFG = getDomainConfig(DETECTED_DOMAIN);
console.log("[DOMAIN] Detected:", DETECTED_DOMAIN, "| Type:", DOMAIN_CFG.type, "| Version:", DOMAIN_CFG.version);
...
if (!PUSHER_ENABLED && process.env.PUSHER_APP_ID) {
  console.warn("[PUSHER] Pusher misconfigured — check PUSHER_APP_ID, PUSHER_KEY, PUSHER_SECRET in .env");
}
...
function resolveApiModel(publicModelName, providerId) {
  ...
  if (providerId && PROVIDER_CONFIG[providerId]) {
    ...
    // প্রথমে name অনুসারে search
    for (const m of p.models) {
      if (getModelName(m) === publicModelName) return getApiModelName(m);
    }
    // না পেলে apiModel অনুসারে search (যেমন deepseek-v4-flash-free → model-pro)
    for (const m of p.models) {
...
const PROVIDER_CONFIG = {
  opencode: {
    ...
    type: "openai",
    models: [
      // OpenCode native model names — no alias needed, these are the real model IDs!
  ...
  // ── Groq (Secondary Fallback — "দালাল") ──────────────────
  // ভাইয়ার ভাষায়: "ওরা শুধু দালালি করে" — মানে পাইপলাইন দেয়, নিজেরা জানে না।
  // OpenCode fail করলে Groq fallback হিসেবে কাজ করবে।
  // priority 2 — OpenCode (priority 1) এর পরেই try করে।
  groq: {
    name: "Groq",
    ...
    const req = proto.request(options, (res) => {
      ...
      res.on("end", () => {
        ...
        if (statusCode < 200 || statusCode >= 300) {
          ...
          // 🧟 RATE LIMIT DETECTION: শুধু HTTP 429 ট্র্যাক করে
          // message-based regex সরিয়েছি — কারণ OpenCode-তে false positive হচ্ছিল
...
// ══════════════════════════════════════════════════════════════
// HTTP 429 rate limit ট্র্যাক করে এবং fallback provider-এ সুইচ করে।
// প্রতিটি domain-এর জন্য আলাদা rate limit state থাকে।
// limit রিসেট না হওয়া পর্যন্ত cooldown period মেনে চলে।
// ══════════════════════════════════════════════════════════════
...
function getRateLimitStatus(domain) {
  ...
  return {
    ...
    maxRateLimit: DOMAIN_CFG.maxRateLimitPerServer,
    message: `🧟 [${domainKey}] ${state.provider === "opencode" ? "OpenCode" : state.provider} rate limit (HTTP 429)। ⏳ ${Math.ceil(remaining / 1000)} সেকেন্ড পর আবার চেষ্টা করুন।`,
  };
}
...
function getCorsOrigin(reqOrigin) {
  ...
  if (allowed) {
    ...
    if (reqOrigin.startsWith("http://localhost") || reqOrigin.endsWith(".my.id") || reqOrigin.endsWith(".kingwin360.com") || reqOrigin.endsWith(".skilltoearn.org"))
      return reqOrigin;
  }
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    if (url === "/api/rate-limit/reset" && method === "POST") {
      ...
      jsonResponse(res, 200, { success: true, message: "Rate limit state cleared for " + requestDomain });
      return;
    }
...
async function init() {
  ...
  server.listen(PORT, "0.0.0.0", () => {
    log("INFO", "START", { port: PORT, agents: AGENTS.length, domain: DETECTED_DOMAIN });
    console.log("\nMission Barisal v3");
    ...
    console.log("  Rate Limit: " + DOMAIN_CFG.maxRateLimitPerServer + " req/min");
    ...
    console.log("  GET  /v1/models          — List agents (masked consumer models)");
    console.log("  GET  /api/v1/models      — List ALL provider models (unmasked, dev)");
    console.log("  POST /v1/chat/completions — Agent or mission");
    console.log("  POST /api/mission        — Full debate mission");
    console.log("  POST /api/v1/anti-dote   — Anti-Dote Type Safety (6-step chain)");
    console.log("  POST /api/mcp-clients    — Connected MCP clients (JSON)");
    console.log("  GET  /api/clients         — Connected MCP clients (HTML)");
    console.log("  POST /api/set-working-dir — Set working dir (for zombieBridge)");
