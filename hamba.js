...
const ALLOWED_ORIGINS = (
  ...
  "http://localhost:3000,http://localhost:5010,http://localhost:7799,http://zombiecoder.my.id,https://api.kingwin360.com,http://m.skilltoearn.org"
)
  .split(",")
  .map((o) => o.trim());
...
// 🧟 ZOMBIE FIX: providerId প্যারামিটার যোগ করা হয়েছে — শুধু নির্দিষ্ট provider-এর
// মধ্যে model search করে। আগে সব provider-এ search করত, যার কারণে Groq-এর
// fallback alias নাম OpenCode-র primary path-এ চলে যেত (ভুল apiModel)।
function resolveApiModel(publicModelName, providerId) {
  // 1. নির্দিষ্ট provider-এর মধ্যে search (যদি providerId দেওয়া থাকে)
  if (providerId && PROVIDER_CONFIG[providerId]) {
    ...
    // ওই provider-এ না পেলে original নাম return
    return publicModelName;
  }
  // 2. Fallback: সব provider-এ search (পুরোনো behaviour)
...
const PROVIDER_CONFIG = {
  ...
  // ── Groq (Primary Fallback — "দালাল") ────────────────────
  // ভাইয়ার ভাষায়: "ওরা শুধু দালালি করে" — মানে পাইপলাইন দেয়, নিজেরা জানে না।
  // OpenCode-র ফ্রি মডেল fail করলে Groq fallback হিসেবে কাজ করবে।
  // priority 2 রাখা হয়েছে যাতে সরাসরি OpenCode-র পরেই try করে।
  groq: {
    name: "Groq",
    ...
    models: [
      ...
      { name: "big-pickle", apiModel: "mixtral-8x7b-32768" },
  ...
  // ── Gemini (Tertiary) ─────────────────────────────────────
  // API key না থাকলে কাজ করে না — শুধু fallback-এর শেষ চেষ্টা
  gemini: {
    name: "Gemini",
    baseUrl:
...
// যদি primary provider ব্যর্থ হয়, callModel পরবর্তী provider চেষ্টা করে
...
// current provider ব্যর্থ হলে পরবর্তী provider চেষ্টা করে
...
function callModel(
  ...
  return new Promise((resolve) => {
    ...
    // 🧟 RATE LIMIT PRE-CHECK: provider limit খেয়েছে কিনা দেখি
    // Limit active থাকলে সরাসরি fallback provider-এ চলে যায়
    if (isRateLimited(providerId, model) && !providerOverride) {
      ...
      if (nextProvider) {
        log("INFO", "RATE_LIMIT_BYPASS", {
          ...
          model,
          cooldown: RATE_LIMIT_STATE.cooldownMs + "ms",
        });
        resolve(
          callModel(
            model,
    ...
    const req = proto.request(options, (res) => {
      ...
      res.on("end", () => {
        ...
        if (statusCode < 200 || statusCode >= 300) {
          ...
          // 🧟 RATE LIMIT DETECTION: 429 বা rate limit error message → ট্র্যাক করে
          const isRateLimit = statusCode === 429 ||
            /rate.?limit|quota|too many|limit.*exceed|exhausted|429/i.test(errMsg);
          if (isRateLimit) {
            setRateLimited(providerId, model, errMsg);
            // Rate limit detected — পরবর্তী provider try করে
            const nextProvider = findNextProvider(model, providerId);
            if (nextProvider) {
          ...
          // Fallback: পরবর্তী provider try করে (non-rate-limit error)
          const nextProvider = findNextProvider(model, providerId);
          if (nextProvider) {
  ...
  open_browser: {
    ...
    params: {
      target: {
        type: "string",
        description:
          "File path or URL to open in the browser (e.g., /abs/path/file.html or http://localhost:5010)",
      },
    },
    required: ["target"],
  },
...
function handleMCP(req, res) {
  ...
  readBody(req).then((body) => {
    ...
    if (method === "initialize") {
      ...
      // ── 🧟 MCP Client Roots Detection (বংশবিস্তার!) ─────────────
      // দালালি করে client-এর project directory চিনে নেওয়ার জায়গা
      ...
      // ── 🧟 Auto SSOT Generation (পদচিহ্ন রেখে আসি!) ─────────────
      let ssotResult = null;
      if (detectedDir) {
        ...
        log("INFO", "MCP_AUTO_SSOT", {
          ...
          message:
            "বংশবিস্তার! .zombiecoder/SSOT.md planted in " + detectedDir,
        });
      }
      ...
      log("INFO", "MCP_CLIENT_CONNECTED", {
        ...
        message: hasRoots
          ? "Client '" +
            ...
            detectedDir + "/.zombiecoder/SSOT.md"
          : "Client '" +
            effectiveName +
      ...
      if (detectedDir) {
        ...
        if (clientSSOT) {
          ...
          ssotInfo =
            "Roots detected but SSOT generation pending for: " +
            detectedDir;
        }
      } else {
    ...
    if (method === "roots/list_changed" || method === "notifications/roots/list_changed") {
...
// ══════════════════════════════════════════════════════════════
//  🧟 RATE LIMIT TRACKING SYSTEM — OpenCode Provider Monitor
// ══════════════════════════════════════════════════════════════
// OpenCode-র ফ্রি মডেলের rate limit শেষ হলে ট্র্যাক করে, clear message দেখায়,
// এবং Groq fallback-এ সুইচ করে। limit রিসেট না হওয়া পর্যন্ত cooldown period
// মেনে চলে — API-তে অহেতুক request পাঠায় না।
// ══════════════════════════════════════════════════════════════

const RATE_LIMIT_STATE = {
  limited: false,       // true = rate limit active
  provider: null,       // কোন provider limit খেয়েছে (যেমন "opencode")
  model: null,          // কোন মডেল limit খেয়েছে
  detectedAt: null,     // কখন limit ডিটেক্ট হয়েছে
  cooldownMs: parseInt(process.env.RATE_LIMIT_COOLDOWN || "120000", 10), // 2 মিনিট cooldown
  originalMessage: null, // original error message from provider
};

function isRateLimited(providerId, model) {
  // কোন limit state না থাকলে false
  if (!RATE_LIMIT_STATE.limited) return false;
  // cooldown period শেষ হলে auto-reset
  if (Date.now() - RATE_LIMIT_STATE.detectedAt > RATE_LIMIT_STATE.cooldownMs) {
    RATE_LIMIT_STATE.limited = false;
    RATE_LIMIT_STATE.provider = null;
    RATE_LIMIT_STATE.model = null;
    return false;
  }
  // নির্দিষ্ট provider/model-এর জন্য limit check
  if (providerId && RATE_LIMIT_STATE.provider !== providerId) return false;
  if (model && RATE_LIMIT_STATE.model && RATE_LIMIT_STATE.model !== model) return false;
  return true;
}

function setRateLimited(providerId, model, errorMessage) {
  RATE_LIMIT_STATE.limited = true;
  RATE_LIMIT_STATE.provider = providerId;
  RATE_LIMIT_STATE.model = model;
  RATE_LIMIT_STATE.detectedAt = Date.now();
  RATE_LIMIT_STATE.originalMessage = errorMessage;
...
function getRateLimitStatus() {
  ...
  const elapsed = Date.now() - RATE_LIMIT_STATE.detectedAt;
  const remaining = Math.max(0, RATE_LIMIT_STATE.cooldownMs - elapsed);
  return {
    limited: true,
    provider: RATE_LIMIT_STATE.provider,
    model: RATE_LIMIT_STATE.model,
    remainingMs: remaining,
    remainingSec: Math.ceil(remaining / 1000),
    detectedAt: new Date(RATE_LIMIT_STATE.detectedAt).toISOString(),
    message: `ভাইয়া! ${RATE_LIMIT_STATE.provider === "opencode" ? "OpenCode" : RATE_LIMIT_STATE.provider}-এর দৈনিক লিমিট শেষ! ⏳ প্রায় ${Math.ceil(remaining / 1000)} সেকেন্ড পর আবার চেষ্টা করুন।`,
  };
}
...
function getCorsOrigin(reqOrigin) {
  ...
  // 🧟 TIGHTENED ORIGIN CHECK: শুধুমাত্র নির্দিষ্ট origin-দের অনুমতি
  // ALLOWED_ORIGINS থেকে সরাসরি matched origin return করে
  const allowed = ALLOWED_ORIGINS.find(
    (o) => reqOrigin === o || reqOrigin.startsWith(o + "/") || reqOrigin.startsWith(o + ":")
  );
  if (allowed) {
    // Exact match বা port variation → origin return
    const exactMatch = ALLOWED_ORIGINS.find((o) => reqOrigin === o);
    if (exactMatch) return exactMatch;
    // Sub-path or port variation → still allow (but use request origin)
    if (reqOrigin.startsWith("http://localhost") || reqOrigin.endsWith(".my.id") || reqOrigin.endsWith(".kingwin360.com"))
      return reqOrigin;
  }
  // Fallback: সবচেয়ে কাছের origin
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    if (url === "/api/rate-limit/reset" && method === "POST") {
      ...
      jsonResponse(res, 200, { success: true, message: "Rate limit state cleared" });
      return;
    }
